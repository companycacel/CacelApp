...
public partial class MantPesajesModel : ViewModelBase
{
    ...
    public async Task InicializarAsync(Pes? pesaje = null, string? tipo = null)
    {
        ...
        {
            Cleanup();
            int ? _pes_mov_id = null;
            if (pesaje.pes_mov_des is not null)
            {
                _pes_mov_id = pesaje.pes_mov_des.Contains("CE") ? pesaje?.pes_mov_id : null;
            }
            _ = CargarMaterialesAsync(_pes_mov_id);
    ...
    private async Task CargarMaterialesAsync(int? movId)
    {
        try
        {
            var materiales = await _selectOptionService.GetSelectOptionsAsync(SelectOptionType.Material, null, new { _bie_mov_id = movId,bie_tipo=1,_dpp_tipo="IN" });
