...
public partial class ProduccionModel : ViewModelBase
{
        ...
        Core.Repositories.Profile.IUserProfileService userProfileService) : base(dialogService, loadingService)
    {
        ...
        TableColumns = new ObservableCollection<DataTableColumn>
        {
            ...
            new ColDef<ProduccionItemDto> { Key = x => x.pde_nbza, Header = "BALANZA", Width = "0.6*", Priority = 2 },
            new ColDef<ProduccionItemDto> { Key = x => x.pes_veh_id, Header = "MAQUINARIA", Width = "0.8*", Priority = 2 },
